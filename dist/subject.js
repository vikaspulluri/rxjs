...
function example1() {
    ...
    sub.next(1);
    sub.subscribe(x => {
        (0, logger_1.log)(`Subscriber A, ${x}`);
    });
    sub.next(2); // OUTPUT => Subscriber A 2
    sub.subscribe(x => {
        (0, logger_1.log)(`Subscriber B, ${x}`);
    });
...
function example2() {
    ...
    const behaviorSubject = new rxjs_1.BehaviorSubject('a');
    const replaySubject = new rxjs_1.ReplaySubject(2);
